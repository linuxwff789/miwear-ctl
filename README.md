...
| 通知 | 标题 / 内容 / 包名 / 推送 | 普通通知 |
| **快应用管理** | 包名 / 指纹(可空) / 启动 URI(可空) | |
| | **列出应用** | `module 20 sub 0`，把已装快应用（含指纹）打到日志 |
| | **查询** | `module 20 sub 21`，查单个应用状态 |
| | **卸载** | 二次确认；设备不回执，看手表界面 |
| | **启动** | 在手表上启动快应用（可带 URI） |
...
| 读什么 | module/sub | 实测结果（REDMI Watch 5） |
|---|---|---|
| 电量 / 充电 | `2/1` | 68%；未充电；已充满；最近充电 9/10 |
| 设备信息 | `2/2` | SN `59685/B6AFMH5SA28800`、固件 `3.100.038`、型号 `M2427W1` |
| 存储 | `2/62` | `f44{f1=33.6MB, f2=311MB}` |
| 心率监测设置 | `8/10` | `f8{f4=100, f8{f2=50}, f10=2, f11=1}` |
| 血氧监测设置 | `8/8` | `f7{f1=1}`（开） |
| 压力监测设置 | `8/14` | `f10{f1=0, f2{f2=80}}` |
...
> `probe` 会把应答打成 protobuf 树（能识别嵌套消息/字符串/float），方便你自己探其他字段。
> ⚠️ 注意：字段号相同的不同类型可能用 int32/uint32/**sint32(zigzag)** 编码，
> 比如 `8/29` 里那个 64 其实是 zigzag → 32（而且它根本不是电量）。
